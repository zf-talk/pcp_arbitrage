...
class AppConfig:
    ...
    signal_ui: str = "classic"       # "classic" | "dashboard" (TTY only for dashboard)
    signal_dashboard_max_rows: int = 30
...
def load_config(path: Path | str = "config.yaml") -> AppConfig:
    ...
    signal_ui = str(arb.get("signal_ui", "classic")).strip().lower()
    if signal_ui not in ("classic", "dashboard"):
        signal_ui = "classic"
    try:
        signal_dashboard_max_rows = int(arb.get("signal_dashboard_max_rows", 30))
    except (TypeError, ValueError):
        signal_dashboard_max_rows = 30
    signal_dashboard_max_rows = max(1, signal_dashboard_max_rows)
    ...
    web_dashboard_width = int(web_cfg.get("width", 220))
    ...
    return AppConfig(
        ...
        signal_ui=signal_ui,
        signal_dashboard_max_rows=signal_dashboard_max_rows,
        ...
        web_dashboard_width=web_dashboard_width,
